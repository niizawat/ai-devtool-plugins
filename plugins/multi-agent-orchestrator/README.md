...
このリポジトリで開発しつつ、Cursor からはローカルプラグインとして参照できるようにします。以下のコマンドは**リポジトリのルート**で実行してください。

### 推奨: シンボリックリンクで有効化（macOS/Linux）

```bash
mkdir -p "$HOME/.cursor/plugins/local"
ln -s "$(pwd)/plugins/multi-agent-orchestrator" \
...
```bash
mkdir -p "$HOME/.cursor/plugins/local"
cp -R "$(pwd)/plugins/multi-agent-orchestrator" \
  "$HOME/.cursor/plugins/local/multi-agent-orchestrator"
```
